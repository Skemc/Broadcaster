...
class UserValidations {
   static validateSignup = (body) => {
      const schema = Joi.object({
         firstName: Joi.string().required().error(new Error('First name of user is required and not allowed to empty')),
         lastName: Joi.string().required().error(new Error('Second name of user is required and not allowed to empty')),
