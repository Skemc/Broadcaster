...
describe('reports tests', function () {
  ...
  it("User should be able to edit red-flag location", function (done) {
    _chai["default"].request(_app["default"]).patch("/api/v1/red-flags/location/".concat(1)).set('auth', _mockdata["default"].rightToken.token).send(_mockdata["default"].editLocation).end(function (err, res) {
      res.should.have.status(200);
      res.body.should.be.an("object");
      done();
    });
